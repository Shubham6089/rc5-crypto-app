...
const CanvasVisualizer = ({ simulationData }) => {
  ...
  return (
    <div className="glass-panel">
      <h2>👁️ Internal Visualization (ECB Mode)</h2>
      <div className="canvas-wrapper">
        <canvas ref={canvasRef} width={800} height={400} />
      </div>
      <div className="canvas-controls">
        <button className="btn-outline" onClick={() => setStep(Math.max(-1, step - 1))}>
          ◀ Previous Step
        </button>
        <button className="btn-outline" onClick={() => setStep(Math.min(simulationData.rounds.length - 1, step + 1))}>
          Next Step ▶
        </button>
      </div>
    </div>
